...
def mock_dependencies():
    ...
    """
    with (
        patch("parallax.probe_detection.yolo_local.yolo_client.YoloKeypoints") as MockWorkerClass,
...
@pytest.fixture
def sample_config():
    return {"fps": 30, "yolo": {"img_dim": [320, 320], "bbox_margin": 50, "mask_margin": 20, "apply_mask": True}}
...
def test_initialization_config_parsing(mock_dependencies, sample_config):
    ...
    MockWorkerClass.assert_called_once_with(
        "LocalClient", sample_config["yolo"], detection_callback=callback, finished_callback=finished_cb
    )
