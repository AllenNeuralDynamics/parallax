...
def test_get_projected_points(mock_params):
    # Simple 3D points
    objpoints = np.array([[0.0, 0.0, 100.0], [1.0, 1.0, 101.0]], dtype=np.float32).reshape(  # Point in front of camera
        2, 1, 3
    )  # Test with (N, 1, 3) input shape
...
def test_get_origin_xyz(imgpoints_data, mock_params):
    ...
    origin, x, y, z = get_origin_xyz(
        ...
        mtx=mock_params.mtx,
        dist=mock_params.dist,
        rvecs=mock_params.rvec,
        tvecs=mock_params.tvec,
