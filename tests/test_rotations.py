...
class RotationsTest(unittest.TestCase):
    ...
    def test_rotate_about(self) -> None:
        ...
        pt = np.array([1, 2, 3])
        pivot = np.array([2, 3, 4])
        R = rotations.define_euler_rotation(np.pi * 2, np.pi * 2, np.pi * 2, degrees=False)
        X = rotations.rotate_about(pt, R, pivot)
        ...
        X = rotations.rotate_about_and_translate(pt, R, pivot, np.array(translate))
    ...
    def test_scipy_rotation_to_sitk(self) -> None:
        ...
        trans = rotations.scipy_rotation_to_sitk(R, center=center, translation=translation)
    ...
    def test_rotation_matrix_from_vectors(self) -> None:
        ...
        rot_mat = rotations.rotation_matrix_from_vectors(a, -a)
        self.assertTrue(np.allclose(rot_mat, -np.eye(a.size)))
        self.assertRaises(ValueError, rotations.rotation_matrix_from_vectors, a, np.zeros(1))
    ...
    def test_prepare_data_for_homogeneous_transform(self) -> None:
        ...
        self.assertTrue(np.array_equal(test_array[:, 0:3], np.array([[1, 2, 3], [4, 5, 6]])))
