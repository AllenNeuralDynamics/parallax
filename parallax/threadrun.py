...
class Future(QtCore.QObject):
    ...
    """
    sigFinished = QtCore.pyqtSignal(object)  # self
    sigStateChanged = QtCore.pyqtSignal(object, object)  # self, state
...
class ThreadCallFuture(Future):
    sigRequestCall = QtCore.pyqtSignal()
    def __init__(self, thread, func, *args, **kwds):
        ...
        if thread is None:
            thread = QtWidgets.QApplication.instance().thread()
