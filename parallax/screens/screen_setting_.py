...
class ScreenSetting(QWidget):
    ...
    def __init__(self, parent, model, screen):
        ...
        # Add setting button
        self.dummy_mode = model.dummy
        self.parent = parent
        self.screen = screen
        self.sn = self.screen.get_camera_name()  # self.sn can be updated on runtime when camera is changed
        self.camera_setting = self.model.config.cameras.get(self.sn)  # CameraSettings schema
        if self.camera_setting is None:
            self.camera_setting = CameraSettings(customName=self.sn)  # TODO get from hardware defaults instead of just using S/N as custom name

        print("Initializing ScreenSetting with model config:", self.camera_setting)
        ...
        self._update_setting_menu()  # Initial sync of the setting menu with the current camera settings
    ...
    def _update_setting_menu(self):
        """Update the settings menu using the validated Config object."""
        self.sn = self.screen.get_camera_name()
        camera_settings = self.model.config.cameras.get(self.sn)
        
        if camera_settings:
            # Pydantic ensures these are the correct types and in range
            
            # FPS & Frame Rate
            self.settingMenu.fpsSlider.setValue(int(camera_settings.fps))
            self.settingMenu.fpsNum.setNum(int(camera_settings.fps))
            
            # Exposure (UI uses ms, converted from yaml)
            self.settingMenu.expSlider.setValue(int(camera_settings.exposureTime_ms))
            self.settingMenu.expNum.setNum(int(camera_settings.exposureTime_ms))
            
            # Gain
            self.settingMenu.gainSlider.setValue(int(camera_settings.gain))
            self.settingMenu.gainNum.setNum(int(camera_settings.gain))
            
            # Gamma
            self.settingMenu.gammaSlider.setEnabled(camera_settings.gammaEnable)
            self.settingMenu.gammaSlider.setValue(camera_settings.gamma)
            self.settingMenu.gammaNum.setText("{:.2f}".format(camera_settings.gamma / 100))

            # White Balance (Only if Color camera)
            if self.screen.get_camera_color_type() == "Color":
                self.settingMenu.wbSliderRed.setValue(camera_settings.wbRed)
                self.settingMenu.wbSliderBlue.setValue(camera_settings.wbBlue)
                self.settingMenu.wbNumRed.setText("{:.2f}".format(camera_settings.wbRed / 100))
                self.settingMenu.wbNumBlue.setText("{:.2f}".format(camera_settings.wbBlue / 100))
            
            # Initialize Bottleneck Warning Color
            if camera_settings.exposureTime_ms > (1000.0 / camera_settings.fps):
                self.settingMenu.fpsNum.setStyleSheet("color: red;")
            else:
                self.settingMenu.fpsNum.setStyleSheet("color: white;")
        else:
            # Fallback for new cameras not yet in YAML
            logger.info(f"No configuration found for {self.sn}, using defaults.")
    ...
    def _show_settings_menu(self, is_checked):
        ...
        if is_checked:
            ...
            UserSettingsManager.save_settings(self.model.config)  # Save the updated camera settings to disk

    def _sync_ui_to_model(self):
        """Pull values from UI widgets and push them into the model reference."""
        if not self.camera_setting:
            return

        # self.camera_setting is a reference to self.model.config.cameras[sn]
        ...
        self.camera_setting.wbRed = int(self.settingMenu.wbSliderRed.value())
        self.camera_setting.wbBlue = int(self.settingMenu.wbSliderBlue.value())  
    ...
    def _set_exposure_auto(self, mode: str):
        """Set the exposure to auto mode based on the camera's current setting."""
        if mode == "Once":
            self.settingMenu.expSlider.setEnabled(False)
            self.screen.set_camera_auto_setting(setting="exposure", mode="Once")
        elif mode == "Off":
            self.settingMenu.expSlider.setEnabled(True)
            self.screen.set_camera_auto_setting(setting="exposure", mode="Off")
        elif mode == "Continuous":
            # Disable the slider while in continuous auto mode since user input won't have an effect
            self.settingMenu.expSlider.setEnabled(False)
            self.screen.set_camera_auto_setting(setting="exposure", mode="Continuous")

    def _set_gain_auto(self, mode: str):
        """Set the gain to auto mode based on the camera's current setting."""
        if mode == "Once":
            self.settingMenu.gainSlider.setEnabled(False)
            self.screen.set_camera_auto_setting(setting="gain", mode="Once")
        elif mode == "Off":
            self.settingMenu.gainSlider.setEnabled(True)
            self.screen.set_camera_auto_setting(setting="gain", mode="Off")
        elif mode == "Continuous":
            # Disable the slider while in continuous auto mode since user input won't have an effect
            self.settingMenu.gainSlider.setEnabled(False)
            self.screen.set_camera_auto_setting(setting="gain", mode="Continuous")
    ...
    def _custom_name(self):
        def on_name_changed():
            new_name = self.settingMenu.customName.text()
            if self.camera_setting:  # Update to model
                self.camera_setting.customName = new_name
            self._update_groupbox_name(self.parent, new_name) # Update GUI Visuals
        self.settingMenu.customName.textChanged.connect(on_name_changed)

    def _custom_name_depre(self):
        """Setup custom name settings in the settings menu."""
        # Set GUI - Custom name
        customName = self.camera_setting.customName
        self.settingMenu.customName.setText(customName)
        self._update_groupbox_name(self.parent, customName)

        # Connect - Update GroupBox name
    ...
    def _update_setting_menu_(self):
