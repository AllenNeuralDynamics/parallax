...
class CameraTransform(coorx.CompositeTransform):
    def __init__(self):
        ...
        super().__init__([self.view, self.proj, self.screen])

    def set_camera(self, center, look, fov, screen_size, up=(0, 0, 1)):
...
class GraphicsView3D(pg.GraphicsView):
    def __init__(self, **kwds):
        ...
        self.camera_params = {'look': [0, 0, 0], 'pitch': 30, 'yaw': 0, 'dist': 10, 'fov': 45}
        super().__init__(**kwds)
    ...
    def update_camera(self):
        ...
        self.camera_tr.set_camera(center=cam_pos, look=look, fov=p['fov'], screen_size=[self.width(), self.height()])
...
def find_checker_corners(img, board_shape, show=False):
    ...
    objp = np.zeros((board_shape[0] * board_shape[1], 3))
...
if __name__ == '__main__':
    ...
    # checkers.transform.set_from_affine(tr)


    # checkers2 = CheckerBoard(win, size=5, colors=[(0, 0, 0, 40), (255, 255, 255, 40)])
    # checkers2.transform = tr
